...
function updateSession(message, listening, connected, protocol) {
    ...
    if (connected) {
        sessionChip.textContent = "Connesso";
        sessionChip.className = "status-chip status-chip--ok";
    } else if (listening) {
        sessionChip.textContent = "In attesa device";
        sessionChip.className = "status-chip status-chip--ok";
    } else {
        sessionChip.textContent = "Disconnesso";
        sessionChip.className = "status-chip status-chip--idle";
    }
    activeProtocol.textContent = protocol || "Nessuno";
    connectionState.textContent = connected ? "Connesso" : listening ? "In ascolto" : "Inattiva";
}
...
async function readValues() {
    setBusy(true);
    try {
        const response = await fetch("/api/read-values", { method: "POST" });
...
connectButton.addEventListener("click", async () => {
    ...
    try {
        ...
        const data = await response.json();
        updateSession(data.message, false, data.isConnected, profile.label);
        updateSocketTarget(data.socketTarget);
        updateRemoteEndpoint(data.remoteEndpoint);
    } finally {
        setBusy(false);
...
disconnectButton.addEventListener("click", async () => {
    ...
    try {
        ...
        const data = await response.json();
        updateSession(data.message, false, false, "");
        updateSocketTarget("");
        updateRemoteEndpoint("");
        state.dataResults = new Map(dataBlocks.map((block) => [block.id, block.result]));
        txLog.textContent = "Nessun comando trasmesso.";
...
window.addEventListener("load", async () => {
    try {
        ...
        const [manifestResponse, stateResponse] = await Promise.all([
            fetch("/protocol-object-map.json", { signal: ac.signal }),
            fetch("/api/state", { signal: ac.signal })
        ]);
        clearTimeout(timer);

        if (!manifestResponse.ok || !stateResponse.ok)
        ...
        sessionMessage.textContent = "Errore caricamento: " + e.message;
        protocolSelect.innerHTML = '<option value="Dlms">DLMS</option><option value="Ctr">CTR</option><option value="Genius">Genius</option><option value="Modbus">Modbus</option>';
        state.manifest = {
            profiles: [
                { id: "Dlms", label: "DLMS", apiProtocol: "Dlms", sections: [{ id: "dati", label: "Dati", kind: "data", operations: ["read"], objects: [] }] },
                { id: "Ctr", label: "CTR", apiProtocol: "Ctr", sections: [{ id: "dati", label: "Dati", kind: "data", operations: ["read"], objects: [] }] }
            ]
        };
        state.profile = "Dlms";
        state.section = "dati";
